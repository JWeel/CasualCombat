...
Later I had the brilliant idea that the *Combat* class did not need a queue. It could simply store one string, and only the *PlayPage* needed a queue. Soon, however, I began seeing that this limited how much information I could give to the user, because I could only show one message per *Move*. How would I handle messages not linked to *Move*s (such as a message for when all *Foe*s are defeated)? A week after my earlier implementation, I reintroduced a queue to the *Combat* class, and rewrote how *PlayPage* interacts with *Combat* (now only urging *Combat* to advance when it did not have any messages left to pop). This coincided with me deciding to make the log window scrollable, allowing all combat messages to be saved (instead of only the most recent). This is way better, because users can scroll up if they forgot what previously happened.
