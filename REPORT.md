...
Later I had the idea that the *Combat* class did not need a queue. It could simply store one string, instead of a queue. Soon, however, I began seeing that this limited how much information I could give to the user, because I could only show one message per *Move* (only one string was stored). How would I handle messages not linked to *Move*s (such as a message for when all *Foe*s are defeated)? A week after this new implementation, I reintroduced a queue to the *Combat* class, and rewrote how *PlayPage* interacts with *Combat* (now only urging *Combat* to advance when it did not have any messages left to pop). This coincided with me deciding to make the log window scrollable, allowing all combat messages to be saved (instead of only the most recent). This is way better, because users can scroll up if they forgot what previously happened.
...
I've definitely experienced that thorough planning goes a long way, and not spending enough time thinking over one concept, in order to save time in the short run (or just because I want to implement it already!), will only cost more time in the long run.

As I look back at my final product, I realize it is not just a fun little game, but that it can serve as a basis for a much larger game. New *Foe*s, *Item*s, and *Move*s can very easily be added, and made to scale with the level of the *PlayerCharacter*. There is currently a "final boss" for players that manage to get through 30 battles. But the game does not stop there. Currently, any *Foe*s faced after that 30th fight will be picked from the same group of potential *Foe*s that the user saw in the previous ten battles. But as new *Foe*s, *Item*s and *Move*s are added, the app will only get better and better at being something that many people seeking some easy and fun time-killing activity (a human problem!) can enjoy.
